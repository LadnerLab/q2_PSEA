qiime psea generate-vis --p-scores-file example/IM0031_PV2T_25nt_raw_2mm_i1mm_Z-HDI75.tsv \
--p-timepoints-file example/timepoints.tsv \
--p-pairs-file example/pairs.tsv \
--p-peptide-sets-file example/input.csv \
--p-species-taxa-file example/species_taxa.tsv \
--p-threshold 0.750000 \
--p-min-size 3 \
--p-max-size 5000 \
--p-permutation-num 10000 \
--p-threads 4 \
--p-table-dir r-ctrl-example-psea-tables \
--output-dir r-ctrl-example \
--p-r-ctrl True
